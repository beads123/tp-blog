                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table id="data-table" class="table table-striped table-bordered" style="text-align:center">
                                    ...
                                    <tbody>
                                    <?php if(is_array($blogInfo)): $i = 0; $__LIST__ = $blogInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                                            ...
                                            <td>
                                                <button class="btn-xs btn-info" data-id="<?php echo ($item["id"]); ?>">查看</button>
                                            </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btn-info').click(function() {
               ...
               $.post("<?php echo U('Index/blog');?>",{'id':id},function(res){
            ...
            $('#permit').live('click', function() {
                var id=$('#showContent').attr('data-id'),
                    _this=this;
                $.post("<?php echo U('Index/UpdateBlogStatus');?>",{'id':id},function(res){
                   var result=$.parseJSON(res);
                   if(result==1){
                        $(_this).text('已通过');
                   }
